...
ShiftRegister74HC595::ShiftRegister74HC595(int dataPin, int latchPin, int clockPin) 
{
...
int ShiftRegister74HC595::_getSlot(int pin) 
{
  int short slot;
...
void ShiftRegister74HC595::_update() 
{
...
void ShiftRegister74HC595::set(int pin, char state) 
{
  int short slot;
  slot = _getSlot(pin);
  double tmpShift = 0;
...
void ShiftRegister74HC595::setAll(char state) 
{
  if (state == HIGH) 
  {
    for (int i = 0; i < NumOfRegisters; i++) 
...
void ShiftRegister74HC595::setNumOfRegisters(int num_of_registers) 
{
